...
ターミナルを起動するとプロンプトに `USER>` と表示されますが、これは接続している仮想の作業環境名である「ネームスペース」の名称が表示されます。
...
詳細は、[ObjectScriptクックブック](/CookBook.md)の[演算子の優先順位：注意点](/CookBook.md#-演算子の優先順位-注意点)をご参照ください。
